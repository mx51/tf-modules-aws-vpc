...
resource "aws_flow_log" "main" {
  ...
  log_destination      = var.send_flow_log_to_external_bucket ? "${var.flow_log_external_bucket}/AWSLogs/${data.aws_caller_identity.current.account_id}/vpcflowlogs/${data.aws_region.current.name}/${aws_vpc.main.id}" : aws_s3_bucket.flow_log[count.index].arn
